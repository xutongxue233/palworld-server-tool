...
自動化、ウォッチドッグ、通知の設定は Web UI から `pst.db` に直接保存されます。ウォッチドッグには、自動検出またはデータベース設定 API で設定された `palworld.control` が必要です。設計では [palworld-server-docker](https://github.com/thijsvanloef/palworld-server-docker) と [TRRabbit Palworld Server Manager](https://github.com/TRRabbit/palworld-server-manager) の運用 UX を参考にしつつ、PST 独自の許可リスト操作、排他制御、送信専用通知を維持しています。

ウォッチドッグは既定で無効、連続失敗しきい値は 3、起動猶予は 90 秒、復旧クールダウンは 120 秒です。これらは **運用 → 自動化** から変更できます。

## マルチサーバーノード

PST 1.8.0 は、複数の Palworld 1.0.0 サーバーを **1 サーバー 1 PST ノード + 中央コントローラー**として管理します。各 PalServer は独立した PST プロセス、`pst.db`、セーブ、バックアップ、スケジューラー、ウォッチドッグ、メンテナンスロックを持ちます。中央 PST は状態を集約し、固定許可リストの既存 API だけを中継します。単一プロセス内でグローバル設定を切り替えないため、別ワールドの処理が混ざらず、危険操作は対象ノード自身のロックと復旧トランザクションで保護されます。
...
管理されるノードでは `PUT /api/setup/config` を使い、識別子と 32 文字以上のランダムな受信用トークンを設定します。
...
中央 PST のデータベース設定にノードを追加します。`id` はリモート `node_id`、`token` はリモート `node_token` と一致させます。

```json
{
  "values": {
    "fleet.node_id": "primary",
    "fleet.node_name": "メインワールド",
...
2. `./pst` を起動して Web UI を開きます。初回アクセス時に Web 管理パスワードを設定して確認すると、そのまま管理モードに入ります。実行中の PalServer、Steam ライブラリ、一般的な導入先を先に検索し、見つからない場合だけパスを手動入力します。

   `save.decode_path` は空のままで構いません。PST はメイン実行ファイルと同じ場所にある `sav_cli` を使用します。

   独自の systemd、Docker、TLS、Fleet 設定は、ログイン後に Swagger から `PUT /api/setup/config` を呼び出します。ドット区切りのキーが `pst.db` に保存されます。
...
`pst_v1.9.1_windows_x86_64.zip`を任意のディレクトリに解凍します（`pst`というディレクトリ名を推奨）。

Windows の `pst.exe` は Linux と同じ Web サービス方式です。コンソールを開いたまま、ログに表示されたアドレスへブラウザーでアクセスします。

初回起動時に Steam レジストリ、すべての `libraryfolders.vdf`、一般的な SteamCMD/SteamLibrary の場所、既存設定から推測できるパスを検索します。明確な候補は自動設定し、同順位の候補だけを選択画面に表示します。見つからない場合のみ PalServer のインストールディレクトリを手動指定します。

##### 設定

通常はパスを手動編集する必要はありません。自動検出結果は `pst.db` に直接保存されます。候補がない場合だけ PalServer の導入先を入力し、独自引数、TLS、Fleet などは認証済みの `GET/PUT /api/setup/config` を使用します。PST の実行中に書き込んだ値は、PST の再起動後に読み込まれます。

`save.decode_path` は空のままで構いません。PST はメイン実行ファイルと同じ場所にある `sav_cli.exe` を使用します。

詳細設定は、認証済みの `PUT /api/setup/config` から `pst.db` に保存します。
...
環境変数はデータベースから読み込んだ実行値を上書きできます。使用できる変数は次のとおりです：
...
バイナリ配置では最初に自動検出を使用します。失敗した場合は Web UI で PalServer の導入先を入力するか、`PUT /api/setup/config` で `save.path` を更新します：
