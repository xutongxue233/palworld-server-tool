...
2. `./pst` を起動し、ログに表示された初期管理パスワードで Web UI にログインします。実行中の PalServer、Steam ライブラリ、一般的な導入先を先に検索し、見つからない場合だけパスを手動入力します。
...
`pst_v1.9.0_windows_x86_64.zip`を任意のディレクトリに解凍します（`pst`というディレクトリ名を推奨）。
