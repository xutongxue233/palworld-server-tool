...
Palworld 1.0.0 仍可使用 RCON，但官方已将其标记为弃用，常用管理建议优先使用 REST API。若需兼容旧服务器或插件命令，请在 `PalWorldSettings.ini` 中设置 `RCONEnabled=True`、确认 `RCONPort`，并配置非空 `AdminPassword`。PST 自动发现会读取这些值并保存到 `pst.db`；修改后可从管理菜单重新扫描。非标准地址可通过认证后的 `PUT /api/setup/config` 更新 `rcon.address`、`rcon.password`、`rcon.use_base64` 与 `rcon.timeout`。
...
自动化、看门狗与通知设置直接通过 Web UI 保存到 `pst.db`。看门狗要求先通过自动发现或数据库配置 API 设置 `palworld.control`。设计参考了 [palworld-server-docker 的定时备份/Discord 通知](https://github.com/thijsvanloef/palworld-server-docker) 和 [TRRabbit Palworld Server Manager 的 Scheduler/Guardian 交互](https://github.com/TRRabbit/palworld-server-manager)，但 PST 保留自己的白名单动作、安全互斥和出站通知边界。

看门狗默认关闭，连续失败阈值为 3，启动宽限 90 秒、恢复冷却 120 秒；这些值均可在“运维 → 自动化”中调整。

## 多服务器节点

PST 1.8.0 使用“**一服一 PST 节点 + 中央控制器**”管理多个 Palworld 1.0.0 服务器。每个 PalServer 仍由独立 PST 进程拥有自己的 `pst.db`、存档目录、备份、定时任务、看门狗和维护锁；中央 PST 只聚合节点状态，并通过固定白名单代理现有 API。这样不同世界可以并行运维，危险操作仍由目标节点自己的互斥锁和恢复事务保护，不会在单进程里切换全局配置造成串服。
...
在被管理节点上通过 `PUT /api/setup/config` 配置身份和至少 32 个随机字符的入站令牌：
...
在中央 PST 的数据库配置中加入远程节点；`id` 必须与远端 `node_id` 一致，`token` 必须与远端 `node_token` 一致：
...
2. 运行 `./pst` 并用浏览器打开 Web 界面。首次打开时，页面会要求设置并确认 Web 管理密码；设置成功后会直接进入管理模式。PST 会先扫描正在运行的 PalServer、Steam 库和常用安装目录；只有没有找到时才需要手动填写安装目录。

   `save.decode_path` 可以留空，PST 会自动使用发布包中与主程序同目录的 `sav_cli`。

   非标准 systemd、Docker、TLS 或 Fleet 设置可在登录后通过 Swagger 调用 `PUT /api/setup/config`。接口使用点号键并写入 `pst.db`，例如：
...
解压 `pst_v1.9.1_windows_x86_64.zip` 到任意目录（推荐命名文件夹目录名称为 `pst`）

Windows 包中的 `pst.exe` 与 Linux 使用相同的 Web 服务方案。双击后保持控制台窗口运行，再通过浏览器访问日志中显示的地址。

首次启动会自动扫描以下位置：

- Steam 注册表安装目录及全部 `libraryfolders.vdf` 库；
- 常见 SteamCMD、SteamLibrary 和独立 PalServer 目录；
- `pst.db` 中已有的存档、INI、SteamCMD 和进程路径。
- 当前正在运行的 `PalServer.exe` 或 Shipping 进程路径。

找到唯一或明显最可信的服务端后，会自动写入 `PalServer.exe`、`PalWorldSettings.ini`、活动世界存档、SteamCMD 安装目录、REST/RCON 端口和进程控制参数。发现多个同等候选时可直接在 Web 界面选择；完全找不到时才需要手动填写包含 `PalServer.exe` 的安装目录。

##### 配置

通常无需手动配置路径。自动发现结果会直接写入解压目录中的 `pst.db`。首次打开 Web 界面时设置管理密码，密码会写入 `pst.db`；以后可从已登录的管理菜单修改，并立即生效。

只有自动扫描失败时才需要在 Web 界面手动填写 PalServer 安装目录。非标准启动参数、TLS、Fleet 等高级设置可通过认证后的 `GET/PUT /api/setup/config` 管理，敏感值不会由读取接口回显；运行期间写入的新值会在重启 PST 后加载。

`save.decode_path` 可以留空，PST 会自动使用发布包中与主程序同目录的 `sav_cli.exe`。

高级设置通过认证后的 `PUT /api/setup/config` 写入 `pst.db`，例如：
...
双击 `start.bat` 或 `pst.exe`，并保持控制台窗口运行。关闭控制台会停止后台任务和 HTTP 服务。
...
当你的 pst 本体通过二进制文件部署时，优先使用自动扫描；扫描失败时在 Web 设置中手动指定 PalServer 安装目录，或通过 `PUT /api/setup/config` 更新 `save.path`：
