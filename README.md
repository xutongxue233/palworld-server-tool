...
2. 运行 `./pst`，从日志获取首次生成的管理密码，再用浏览器打开 Web 界面。PST 会先扫描正在运行的 PalServer、Steam 库和常用安装目录；只有没有找到时才需要手动填写安装目录。
...
解压 `pst_v1.9.0_windows_x86_64.zip` 到任意目录（推荐命名文件夹目录名称为 `pst`）
...
通常无需手动配置路径。自动发现结果会直接写入解压目录中的 `pst.db`。首次启动会生成管理密码并打印到控制台，使用该密码登录 Web 管理模式。
